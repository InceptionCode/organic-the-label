...
export default function CompositionFilters() {
  ...
  const push = useCallback(
    (update: FilterUpdate) => {
      ...
      startTransition(() => {
        router.push(qs ? `/compositions?${qs}` : '/compositions');
      });
    },
    [router, searchParams],
  );
  ...
  const clearAll = () => {
    startTransition(() => router.push('/compositions'));
  };

  return (
    <section
      data-testid="composition-filters"
    ...
    >
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
        <SortSelect
          value={range}
          onChange={(value) => push({ range: value })}
        ...
        <MultiSelectFilter
          ...
          options={TAG_OPTIONS}
          selectedValues={tags}
          onChange={(value) => push({ tags: value })}
          searchable
